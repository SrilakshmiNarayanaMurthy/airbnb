...
export default function Listings() {
  ...
  // 🔹 Redux state
  const items = useSelector(selectListings);            // property data in Redux
  const loading = useSelector(selectListingsLoading);   // loading flag in Redux
  const favIds = useSelector(selectFavorites);          // favourites in Redux

  const today = new Date().toISOString().slice(0, 10);
  const tomorrow = new Date(Date.now() + 86400000).toISOString().slice(0, 10);

  // local UI state (search form, tabs, history list)
  ...
  // history trips (this can stay local)
  const [trips, setTrips] = useState([]);

  // ---- search listings (Redux) ----
  async function search() {
    dispatch(listingsFetchStart());
    ...
    try {
      ...
      dispatch(listingsFetchSuccess(data));
    } catch (e) {
      dispatch(listingsFetchFailure(e.message));
      alert(e.message);
    }
  }

  // ---- load favourites (IDs) into Redux ----
  async function loadFavs() {
    try {
      const ids = await api("/api/favorites"); // [id, id, ...]
      dispatch(setFavorites(ids || []));
    } catch {
      // not logged in -> clear favourites
  ...
  // when user switches to "history" tab, load history
  useEffect(() => {
  ...
  async function book(listingId) {
    ...
    try {
      ...
      // 🔹 store last booking in Redux (booking state)
      dispatch(setLastBooking(data));

      alert(
        `✅ Booking pending! Request sent. Check status under Trips. Total: ${formatMoney(
          data.total_price || 0
        )} for ${data.nights || "?"} night(s).`
      );
    } catch (e) {
      console.error(e);
      if (e?.status === 401) {
  ...
  async function toggleFav(id) {
    try {
      ...
      // 🔹 update favorites in Redux
      dispatch(toggleFavoriteId(id));
    } catch (err) {
      if (err?.status === 401) return alert('Please login to use favourites');
  ...
  const list =
    view === "fav"
      ? items.filter((it) => favIds.includes(it.id))
      : items;
  ...
  return (
    <div className="container" style={{ display: "grid", gap: 20 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button
          className="btn"
          style={view === "history" ? { fontWeight: 700 } : {}}
          ...
          <div className="grid">
            <div className="field">
              ...
              <input
                ...
                value={q.city}
                onChange={onChange}
                placeholder="e.g., Miami or India"
              />
            </div>
      ...
      {view !== "history" && (
        <div className="grid">
          {list.map((it) => {
            const liked = favIds.includes(it.id);
              ...
              >
                ...
                >
                  ...
                  <div style={{ fontWeight: 700 }}>
                    {formatMoney(it.price_per_night)}/night
                  </div>
                </div>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <button
                    className="btn"
                    onClick={() => book(it.id)}
                    disabled={!q.start || !q.end || tooManyGuests}
                  >
                    Book
                  </button>
                  ...
                  {tooManyGuests && (
                    <small style={{ opacity: 0.7 }}>
                      Guests exceed max ({it.max_guests})
                    </small>
                  )}
                </div>
              </div>
          ...
          {trips.map((t) => (
            ...
            >
              <img
                src={
                  t.image_url ||
                  "https://via.placeholder.com/800x500?text=Trip"
                }
                alt={t.title}
                style={{
                  width: "100%",
              ...
              <div style={{ margin: 0 }}>
                Stay: {fmtDate(t.check_in)} → {fmtDate(t.check_out)} ·{" "}
                {t.nights} night(s) · Guests: {t.guests}
              </div>
              <p style={{ margin: 0 }}>{t.description}</p>
            </div>
          ))}
          {trips.length === 0 && (
            <div className="card">No past trips found.</div>
          )}
        </div>
      )}

      {!loading && view !== "history" && list.length === 0 && (
        <div className="card">No listings match your filters.</div>
      )}

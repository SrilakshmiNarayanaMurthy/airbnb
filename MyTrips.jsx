  ...
  return (
    <div className="card" style={{ display: "grid", gap: 12 }}>
      ...
      {rows.length === 0 ? (
        ...
        <div className="grid">
          {rows.map(r => (
            <div className="card" key={r.booking_id} style={{ display: "grid", gap: 10 }}>
              <img
                src={r.image_url || "https://via.placeholder.com/800x500?text=Listing"}
                alt={r.title}
              ...
              <div className="subtle">{r.city}, {r.country}</div>
              <div className="subtle">
                {fmt(r.check_in)} → {fmt(r.check_out)} · {r.nights} night(s) · ${Number(r.total_price || 0).toFixed(2)}
              </div>
            </div>
          ))}
        </div>
...
export default function MyTrips() {
  ...
  const upcoming  = items.filter(r => r.status === "accepted" && r.check_in >= today);
  ...
  return (
    <div className="container" style={{ display: "grid", gap: 16 }}>
      <div style={{ display: "flex", gap: 8 }}>
        <h2 className="title" style={{ margin: 0, flex: 1 }}>My Trips</h2>
